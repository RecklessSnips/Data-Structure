...
public class HeroMinHeap {
    ...
    public HeroMinHeap(Hero[] heroes){
        heap = heroes;
        length = heap.length;
        sortedLength = length;
    }
    ...
    public void heapify(){
        ...
        当前sub tree（也就三个node：parent，left，right）是否构成一个min Heap
        并且以parent交换之后的sub tree开始，重复这个步骤，直到形成一个min heap.
         */
...
//                int min;
                ...
                // min 作为所要替换的node的实际index，需要拿这个node的id跟parent比
                System.out.println("min index: " + min);
                if (parent.getId() > heap[min].getId()){
                ...
                // 只和左边比较, 如果parent小，说明不是min heap
                if (parent.getId() > left.getId()){
                    min = leftIndex;
                    // 开始交换, 注意是在数组里交换，要用index来交换
                    Hero tmp = heap[parentIndex];
                    heap[parentIndex] = heap[min];
                    // 此时的heap[min] 是被换下来的，需要对它继续down heap
                    heap[min] = tmp;
                }
            }
        }
        ...
        if (min != parentIndex){
            downHeap(heap[min], min, heapSize);
        }else{
            System.out.println("已经是 min heap，不需要 Down heap");
        }
    }
...
//        };
//        HeroMinHeap minHeap = new HeroMinHeap(heroes);
